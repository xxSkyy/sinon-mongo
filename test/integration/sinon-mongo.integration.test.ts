...
import sampleController from "./fixtures/sample-express-controller"
import sampleRepository from "./fixtures/sample-repository"
require("../../src/index")
...
describe("sinon-mongo", () => {
  ...
  describe("used in the sample repository", () => {
    ...
    it("is used to test a find operation", () => {
      ...
      return repository
        .findCustomersInOrganization("mockOrgName")
        .then((customers) => {
    ...
    it("is used to test a findOneAndUpdate operation", () => {
      ...
      mockCustomerCollection.findOneAndUpdate
        .withArgs(
          { _id: sinon.match((val) => mockId.equals(val)) },
          { $set: mockUpdates }
        )
        .resolves({ value: mockUpdatedCustomer })

      return repository
        .updateCustomer(mockId, mockUpdates)
